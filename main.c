...
static int packet_transmit(sens_pkt* pkt);
...
//		/* Enable data ready interrupts */
  ...
  if(HAL_UART_Receive_DMA(&huart3, (uint8_t*)Rxdata, 700) != HAL_OK){
	  printf("it not work\r\n");
	  ///GPS
  }
  /*  DP Sensor -------------------------------------------------------*/
  ...
  while (1)
  {
	...
	if (accel_rdy) {
						...
						printf(
								...
								bmi08x_accel.z);
						pkt.stale_bits = pkt.stale_bits | IMU_ACC_NEW;
						pkt.imu_acc.i16bit[0] = bmi08x_accel.x;
						pkt.imu_acc.i16bit[1] = bmi08x_accel.y;
						pkt.imu_acc.i16bit[2] = bmi08x_accel.z;
					...
					if (gyro_rdy) {
						...
						bmi08x_error_codes_print_result("bmi08g_get_data",
								rslt);
						gyro_rdy = 0;
						pkt.stale_bits = pkt.stale_bits | IMU_GYR_NEW;
						pkt.imu_gyr.i16bit[0] = bmi08x_gyro.x;
						pkt.imu_gyr.i16bit[1] = bmi08x_gyro.y;
						pkt.imu_gyr.i16bit[2] = bmi08x_gyro.z;
	...
	if (mag_drdy) {
		...
		pkt.stale_bits = pkt.stale_bits | MAG_NEW;
		pkt.mag.i16bit[0] = data_raw_magnetic[0]; //not sure if this is gonna work how i want it to but maybe
		pkt.mag.i16bit[1] = data_raw_magnetic[1];
		pkt.mag.i16bit[2] = data_raw_magnetic[2];
	...
	if(dp_read){
		DPMeasurement(); ///Dp sensor
		dp_read = false;
	}
	/*  End of Differential Pressure -------------------------------------------------------------*/
	/*  Barometer -------------------------------------------------------------*/

	printf("Altitude Difference: %f\r\n", DPS310_GetPress()); ///barometer

	/*  End of Barometer -------------------------------------------------------------*/
	pkt.stale_bits = packet_transmit(&pkt);
  }
...
static int packet_transmit(sens_pkt* pkt){
	...
	CDC_Transmit_FS((uint8_t *)pkt, sizeof(sens_pkt));
